...
async fn instance<P: AsRef<Path>>(
    path: P,
    keystore: Option<String>,
    passphrase: Zeroizing<String>,
) -> anyhow::Result<(Box<dyn MultiPass>, Box<dyn Constellation>, Box<dyn RayGun>)> {
    ...
    let mut config = warp_ipfs::config::IpfsConfig::production(path, false);
    ...
    let instance =
        WarpIpfsInstance::new(config, tesseract, None).await?;

    //validating that account exist
    let _ = instance.get_own_identity().await?;

    Ok((Box::new(instance.clone()), Box::new(instance.clone()), Box::new(instance)))
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let (account, _fs, rg) = instance(&opt.path, opt.keystore.clone(), password).await?;
    let end_time = start_time.elapsed();
    println!("Took {}ms to load the account, filesystem, and messenging", end_time.as_millis());
