...
use warp_mp_ipfs::{ipfs_identity_persistent, ipfs_identity_temporary, IpfsIdentity};
...
async fn account(
    ...
    opt: &Opt,
) -> anyhow::Result<IpfsIdentity<TestTypes>> {
...
async fn account_persistent<P: AsRef<Path>>(
    ...
    cache: Option<Arc<RwLock<Box<dyn PocketDimension>>>>,
    opt: &Opt,
) -> anyhow::Result<IpfsIdentity<Types>> {
...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "Registered user {}#{} with DID: {}",
        own_identity.username(),
        own_identity.short_id(),
        own_identity.did_key()
    );
