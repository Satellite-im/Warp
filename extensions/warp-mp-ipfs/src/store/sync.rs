    ...
    pub async fn new(ipfs: Ipfs<T>, did: Arc<DID>) -> Result<Self, Error> {
        ...
        tokio::spawn({
            ...
            async move {
                ...
                let peer_id_recipient =
                    did_to_libp2p_pub(&DID::from_str(&did_recipient_string)?)?.to_peer_id();
                let dial_opt = DialOpts::peer_id(peer_id_recipient)
                    .condition(PeerCondition::Disconnected)
                    .addresses(vec![
                        "/ip4/127.0.0.1/tcp/5001".parse().unwrap(),
                        "/ip6/::1/tcp/5001".parse().unwrap(),
                        "/ip4/192.168.1.131/tcp/5001".parse().unwrap(),
                    ])
                            ...
                            request = rx.recv() => { 
                                ...
                                if let Some(request) = request {
                                    ...
                                    match request {
                                        ...
                                        let did_recipient_string = std::env::var("SYNCKEY").unwrap();
                                        println!("SYNC KEY {}", did_recipient_string);
