...
// todo: assumes all fragments have the same time base
// todo: find a way to share code between this and run()
fn write_final_fragments(
    mut fragments: Vec<VecDeque<Mp4Fragment>>,
    mut fragment_sequence_number: u32,
    mut fragment_decode_time: u64,
    mut writer: BufWriter<File>,
    mp4_file_path: PathBuf,
) -> Result<()> {
    while !fragments.is_empty() {
        // find the earliest start time
        let mut fragment_start_time = None;
        for track_idx in 0..fragments.len() {
            if let Some(fragments) = fragments.get_mut(track_idx) {
                if let Some(f) = fragments.front() {
                    match fragment_start_time {
                        None => {
