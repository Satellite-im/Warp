...
impl RayGunCommunity for WarpIpfs {
    ...
    async fn delete_channel(&mut self, community_id: Uuid, channel_id: Uuid) -> Result<(), Error> {
        self.messaging_store()?
            .delete_channel(community_id, channel_id)
            .await
    }
