...
use warp::{
    ...
    raygun::{
        AttachmentEventStream, AttachmentKind, Conversation, ConversationSettings, GroupSettings,
        Location, MessageEvent, MessageEventKind, MessageOptions, MessageReference, MessageStatus,
...
impl ConversationTask {
    ...
    async fn get_message(
        ...
        let keystore = match conversation {
            ConversationDocument::Direct { .. } => None,
            ConversationDocument::Group { .. } => self.get_keystore(conversation.id()).await.ok(),
        };
        conversation
    ...
    pub async fn get_messages(
        ...
        let keystore = match conversation {
            ConversationDocument::Direct { .. } => None,
            ConversationDocument::Group { .. } => self.get_keystore(conversation_id).await.ok(),
        };
    ...
    pub async fn message_status(
        ...
        if matches!(conversation, ConversationDocument::Group { .. }) {
...
impl ConversationTask {
    ...
    pub async fn add_restricted(
        ...
        if matches!(conversation, ConversationDocument::Direct { .. }) {
            return Err(Error::InvalidConversation);
        }
    ...
    pub async fn update_conversation_name(
        ...
        let settings = match &conversation {
            ConversationDocument::Group { settings, .. } => *settings,
    ...
    pub async fn update_conversation_settings(
        ...
        let group_settings = match settings {
            ConversationSettings::Group(settings) => settings,
            _ => return Err(Error::InvalidConversation),
        };

        let conversation_settings = match &mut conversation {
            ConversationDocument::Group { settings, .. } => settings,
            _ => return Err(Error::InvalidConversation),
        };

        *conversation_settings = group_settings;
        self.set_document(conversation).await?;

        let conversation = self.get(conversation_id).await?;
        let event = MessagingEvents::UpdateConversation {
            conversation: conversation.clone(),
            kind: ConversationUpdateKind::ChangeSettings { settings },
        };
...
async fn message_event(
    ...
    let keystore = match &document {
        ConversationDocument::Direct { .. } => None,
        ConversationDocument::Group { .. } => this.get_keystore(conversation_id).await.ok(),
    };
        ...
        MessagingEvents::UpdateConversation {
            ...
            match kind {
                ConversationUpdateKind::AddParticipant { did } => {
                    ...
                    conversation.excluded = document.excluded.clone();
