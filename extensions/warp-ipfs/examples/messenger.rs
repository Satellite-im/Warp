...
async fn create_account_and_chat<P: AsRef<Path>>(
    path: Option<P>,
    cache: Arc<RwLock<Box<dyn PocketDimension>>>,
    passphrase: Zeroizing<String>,
    experimental: bool,
    opt: &Opt,
) -> anyhow::Result<(Box<dyn MultiPass>, Box<dyn RayGun>)> {
    ...
    let mut config = match path.as_ref() {
        Some(path) => warp_ipfs::config::IpfsConfig::production(path, experimental),
        None => warp_ipfs::config::IpfsConfig::testing(experimental),
    };
    ...
    let instance = WarpIpfsInstance::new(config, tesseract, Some(cache)).await?;

    let mut account: Box<dyn MultiPass> = Box::new(instance.clone());

    if account.get_own_identity().await.is_err() {
        account.create_identity(None, None).await?;
    }

    let raygun = Box::new(instance) as Box<_>;
    Ok((account, raygun))
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let (new_account, mut chat) = create_account_and_chat(
        opt.path.clone(),
        cache.clone(),
    ...
    // let _fs = create_fs(new_account.clone(), opt.path.clone()).await?;
