...
use warp_ipfs::{WarpIpfsInstance, config::IpfsConfig};
...
async fn instance_persistent<P: AsRef<Path>>(
    username: Option<&str>,
    path: P,
    opt: &Opt,
...
async fn main() -> anyhow::Result<()> {
    ...
    let (_account, mut filesystem) = instance_persistent(None, opt.path.clone(), &opt).await?;
