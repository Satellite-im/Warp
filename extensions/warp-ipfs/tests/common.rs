...
pub async fn create_account_and_chat(
    ...
    let mut config = warp_ipfs::config::IpfsConfig::development();
    ...
    let instance = WarpIpfsInstance::new(config, tesseract, None).await?;

    let mut account = Box::new(instance.clone()) as Box<dyn MultiPass>;
