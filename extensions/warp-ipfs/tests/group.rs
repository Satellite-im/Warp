...
mod test {
    ...
    use warp::{
        ...
        raygun::{
            ConversationType, GroupPermission, GroupPermissions, MessageEventKind, RayGunEventKind,
        },
    };
    ...
    async fn create_empty_group_conversation() -> anyhow::Result<()> {
        ...
        instance_a
            .create_group_conversation(None, vec![], GroupPermissions::new())
            .await?;
    ...
    async fn add_recipient_to_conversation_to_open_conversation() -> anyhow::Result<()> {
        ...
        permissions.insert(
            did_b.clone(),
            vec![GroupPermission::AddParticipants].into_iter().collect(),
        );
        permissions.insert(
            did_c.clone(),
            vec![GroupPermission::AddParticipants].into_iter().collect(),
                ...
                tokio::select! {
                    Some(event) = chat_subscribe_a.next() => {
                        match event {
                            ...
                            RayGunEventKind::CommunityCreated { .. } => unreachable!(),
                        }
                    }
                }
